...
def write_vector_by_trains(cls, trains, data):
    ...
    with cls('tmpwork/test_{seq:02d}.h5', detname=detname) as wr:
...
def write_vector_h5py(trains, data, chunks):
    ...
    first[0] = 0
    shape = (nrec,) + data.shape[2:]
    with h5py.File('tmpwork/test_plain.h5', 'w') as f:
...
def bench(fun, tag, min_rep=3, max_rep=30, tm_limit=30):
    ...
    pr.dump_stats(f"stats/writer_{tag}.pstats")
    subprocess.run(f"gprof2dot -f pstats stats/writer_{tag}.pstats | "
                   f"dot -Tpng -o stats/writer_{tag}.png", shell=True)
